...
import { TransformStep, WidthTarget, WidthDirection } from './pattern-types';
...
export class TransformProcessor {
  ...
  processTransform(
    ...
    switch (type) {
      case 'text':
        return this.domOperator.createTextNode(result);
      case 'tcy':
        return this.domOperator.createTcyElement(result);
      case 'fragment':
        const temp = this.domOperator.createElement('div');
        temp.innerHTML = result;
        return this.domOperator.createFragment(Array.from(temp.childNodes));
      default:
