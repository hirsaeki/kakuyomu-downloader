...
import type { PatternConfig, PatternLoadingError } from '../types';
import { validatePatternConfig } from './pattern-validation';
 ...
 * プロジェクトルートからの相対パス
 */
export const DEFAULT_PATTERNS_DIR = 'src/lib/text-processing/config/patterns';
    ...
    async load(id) {
      if (id === resolvedVirtualModuleId) {
        ...
        try {
          ...
          // 生成されるモジュールコード
          const result = `import type { PatternConfig } from '@/lib/text-processing/types';
