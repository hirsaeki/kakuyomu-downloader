...
describe('ConvertKanjiStep', () => {
  ...
  describe('execute', () => {
    ...
    it('should throw TransformError for invalid input', async () => {
      ...
      await expect(
        step.execute(createContext('12.34'))
      ).rejects.toThrow(TransformError);

      await expect(
        step.execute(createContext('-123'))
      ).rejects.toThrow(TransformError);
    });
  });
  ...
  describe('edge cases', () => {
    ...
    it('should handle whitespace', async () => {
      await expect(
        step.execute(createContext(' 123 '))
      ).resolves.toEqual({ textContent: '一二三' });
    });
    ...
    it('should reject floating point numbers', async () => {
      await expect(
        step.execute(createContext('1.23'))
      ).rejects.toThrow(TransformError);
    });

    it('should reject scientific notation', async () => {
      await expect(
        step.execute(createContext('1e3'))
      ).rejects.toThrow(TransformError);
    });
