...
import type { TransformContext, WidthDirection, WidthTarget } from '@/lib/typography/core/transform/types';
...
describe('ConvertWidthStep', () => {
  ...
  describe('execute', () => {
    it.each<[WidthTarget, WidthDirection, string, string]>([
      ...
      ['symbols', 'fullwidth', '!@#', '！＠＃'],
      ['symbols', 'halfwidth', '！＠＃', '!@#'],
    ])('should convert %s to %s correctly', async (
      target, direction, input, expected
    ) => {
      const step = new ConvertWidthStep(direction, target);
    ...
    it('should throw TransformError for invalid input', async () => {
      const step = new ConvertWidthStep('fullwidth', 'numbers');
      await expect(
        step.execute(createContext('abc'))
